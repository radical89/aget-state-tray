...
| State | Icon | Meaning |
|---|---|---|
| Running | Green `AI` + VRAM (e.g. `14G`) | llama-server active, model loaded |
...
Left-click calls `systemctl --user stop` or `start` on the service, not `kill`, so systemd's `Restart=on-failure` does not fight against you. Clicks during a transition are silently ignored. A crash loop (SubState `auto-restart`) is treated as a failed state and a click issues `stop` to break the loop.
...
Selecting a model writes the new selection to `~/.config/llama-server/current.env` and restarts the service if it is currently running. The menu re-scans `models_dir` every time it opens, so newly downloaded models appear automatically without restarting the tray.
...
The `models_dir` path supports `~` expansion. Model paths in the menu are relative to this directory.
...
ExecStart=/home/karlos/llama-mtp/build/bin/llama-server --model ${LLAMA_MODEL} $LLAMA_ARGS
```
